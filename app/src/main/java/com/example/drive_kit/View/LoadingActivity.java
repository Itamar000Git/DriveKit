 ...
 *
 * MVVM:
 * - Activity = UI only (observe LiveData + navigation + scheduler triggers)
 * - ViewModel = login + route decision
 * - Repository = FirebaseAuth + Firestore checks
 */
public class LoadingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ✅ NEW: observe route decision and navigate
        viewModel.getRouteDecision().observe(this, decision -> {
            ...
            switch (decision.target) {
                case INSURANCE_HOME: {
                    ...
                    Intent nextIntent = new Intent(this, InsuranceHomeActivity.class);
                    nextIntent.putExtra("insuranceCompanyId", decision.insuranceCompanyId);
                    nextIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    startActivity(nextIntent);
                    finish();
                    break;
                ...
                case SIGN_UP: {
                    ...
                    Intent nextIntent = new Intent(this, SignUpActivity.class);
                    nextIntent.putExtra("fromAuthNoDriverDoc", true);
                    nextIntent.putExtra("prefillEmail", decision.prefillEmail);
                    nextIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    startActivity(nextIntent);
                    finish();
                    break;
        ...
        // Kick off login exactly like before
        if (googleIdToken != null && !googleIdToken.trim().isEmpty()) {
            viewModel.loginWithGoogle(googleIdToken);
        } else {
            viewModel.login(email, password);
