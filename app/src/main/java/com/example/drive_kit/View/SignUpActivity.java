...
public class SignUpActivity extends AppCompatActivity {
    ...
    // Keep same lists so your existing "chosen string -> id" mapping stays unchanged
    private final ArrayList<String> insuranceCompanyIds = new ArrayList<>();
    private final ArrayList<String> insuranceCompanyNames = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ✅ MVVM: Insurance companies list + details
        setupInsuranceCompaniesDropdownMvvm();
    ...
    // ✅ MVVM replacement for BOTH firebase methods
    private void setupInsuranceCompaniesDropdownMvvm() {
        if (insuranceCompanyDropdown == null) return;

        insuranceCompanyDropdown.setOnClickListener(v -> insuranceCompanyDropdown.showDropDown());

        // list observer (fills same local arrays you already use later)
        viewModel.getInsuranceCompanies().observe(this, items -> {
            insuranceCompanyIds.clear();
            insuranceCompanyNames.clear();

            if (items != null) {
                for (SignUpInsuranceCompaniesRepository.CompanyItem it : items) {
                    insuranceCompanyIds.add(it.docId);
                    insuranceCompanyNames.add(it.name);
                }
            }
            ...
            insuranceCompanyDropdown.setOnItemClickListener((parent, view, position, id) -> {
                if (position >= 0 && position < insuranceCompanyIds.size()) {
                    selectedCompanyId = insuranceCompanyIds.get(position);
                    // load details via VM (Firestore not in Activity)
                    viewModel.loadInsuranceCompanyDetails(selectedCompanyId);
                }
            });
        });

        viewModel.getInsuranceCompaniesError().observe(this, msg -> {
            if (msg != null && !msg.trim().isEmpty()) {
                Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
            }
        });

        viewModel.getSelectedCompanyDetails().observe(this, details -> {
            if (details == null) return;
            if (isDriverSelected()) return; // same behavior as before

            // Fill fields exactly like your old method did
            if (emailEditText != null) emailEditText.setText(details.email);
            if (phoneEditText != null) phoneEditText.setText(details.phone);
            if (firstNameEditText != null) firstNameEditText.setText(details.name);
            if (insuranceCompanyIdEditText != null) insuranceCompanyIdEditText.setText(details.hp);
        });

        // initial load
        viewModel.loadInsuranceCompanies();
    }
    ...
    // --- GOV API part remains as-is (unchanged) ---
    private void fetchCarInfoFromGov(String carNumber) {
        final String urlStr =
                "https://data.gov.il/api/3/action/datastore_search" +
                        "?resource_id=053cea08-09bc-40ec-8f7a-156f0677aff3" +
