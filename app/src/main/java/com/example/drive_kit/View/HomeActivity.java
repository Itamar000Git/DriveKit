 ...
 *
 * MVVM:
 * - Activity = UI only (bind views, listeners, observe LiveData)
 * - ViewModel = loads Driver + welcome text via repository
 */
public class HomeActivity extends BaseLoggedInActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Observe welcome text (only once - no duplicate observers)
        homeVm.getWelcomeText().observe(this, vmText -> {
            String t = (vmText == null) ? "" : vmText.trim();
        ...
        // ✅ NEW: observe full Driver and load avatar from it (no Firestore in Activity)
        homeVm.getDriver().observe(this, this::loadAvatarIntoHome);

        // Initial load (welcome text + driver)
        homeVm.loadWelcomeText(uid);
    }
    ...
    protected void onResume() {
        super.onResume();
        // ✅ No re-observing here, no Firestore call here.
        // If you want to refresh when coming back, just ask VM to reload (safe + minimal).
        homeVm.loadWelcomeText(uid);
    }
    ...
    // UI-only avatar loader (same logic you already had, but now uses driver from VM)
    private void loadAvatarIntoHome(Driver d) {
        if (homeAvatar == null) return;
