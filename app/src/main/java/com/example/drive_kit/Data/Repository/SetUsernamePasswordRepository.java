 ...
 *    - Create Firebase Auth user
 *    - Optionally upload local car photo
 *    - Save Driver document in Firestore collection "drivers/{uid}"
 * 2) Insurance registration:
 *    - Create Firebase Auth user
 *    - Mark existing insurance company doc as partner (isPartner=true)
 *    - ✅ NEW: optionally upload insurance logo and save logoUrl in company doc
 */
public class SetUsernamePasswordRepository {
    ...
    // =========================================================
    /**
     * ✅ NEW: insuranceLogoUriLocal is optional (may be null/empty or already http).
     */
    public void registerInsurance(String email,
                                  String password,
                                  Driver insuranceContact,
        ...
        auth.createUserWithEmailAndPassword(safeEmail, safePass)
                .addOnSuccessListener(res -> {
                    ...
                    // 1) claim/update existing company doc
    ...
    /**
     * Updates existing insurance company doc as partner.
     * ✅ NEW: if logoUriLocal provided -> upload + update logoUrl
     */
    private void upsertInsuranceCompany(@NonNull FirebaseUser user,
                                        Driver insuranceContact,
                                        @NonNull String companyId,
                ...
                .addOnSuccessListener(snap -> {
                    ...
                    ref.set(updates, SetOptions.merge())
                            .addOnSuccessListener(v -> {
                                // 2) optionally upload logo and save logoUrl
                                maybeUploadInsuranceLogo(companyId, insuranceLogoUriLocal);
                                cb.onSuccess();
                            })
                            .addOnFailureListener(e -> {
                                // optional: rollback auth user to avoid partial state (same spirit as driver)
        ...
        ref.putFile(uri)
                .addOnSuccessListener(taskSnapshot ->
                        ref.getDownloadUrl()
                                ...
                                .addOnFailureListener(e ->
                                        Log.e("InsuranceSignUp", "getDownloadUrl failed", e))
                )
                .addOnFailureListener(e ->
                        Log.e("InsuranceSignUp", "logo upload failed", e));
    }
