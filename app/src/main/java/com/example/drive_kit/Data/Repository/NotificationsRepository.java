 ...
 * It uses the FirebaseFirestore class to access the database.
 *
 * עדכון: נהג יחיד + רכב יחיד בתוך driver.car
 * לכן כל השדות של תאריכים/דחיות מתעדכנים עם path של "car.<field>"
 *
 * שים לב:
 * - formattedInsuranceDate / formattedTestDate / formattedTreatDate נשארים TOP-LEVEL
 *   כי אמרת שנוח לראות אותם במסמך. (אם תרצה גם בתוך car — אגיד לך מה לשנות)
 */
public class NotificationsRepository {
    ...
    /**
     * Defer the notification.
     * Updates the dismissed stage INSIDE driver.car
     */
    public void deferNotification(String uid, NotificationItem item, SimpleCallback cb) {
        if (uid == null || uid.trim().isEmpty() || item == null) {
            cb.onError(new IllegalArgumentException("uid is null/empty or item is null"));
            return;
        }
        ...
        switch (item.getType()) {
            case INSURANCE:
                dismissedPath = "car.dismissedInsuranceStage";
                break;
            case TEST:
                dismissedPath = "car.dismissedTestStage";
                break;
            case TREATMENT_10K:
                dismissedPath = "car.dismissedTreatment10kStage";
                break;
            default:
                cb.onError(new IllegalArgumentException("Unknown notification type: " + item.getType()));
                return;
        ...
        Map<String, Object> update = new HashMap<>();
        update.put(dismissedPath, item.getStage().name());
    ...
    /**
     * Updates the service date.
     * Updates date INSIDE driver.car and resets dismissed stage INSIDE driver.car
     * Formatted field stays TOP-LEVEL (for convenience / visibility)
     */
    public void doneButton(String uid, NotificationItem.Type type, long newDateMillis, SimpleCallback cb) {
        if (uid == null || uid.trim().isEmpty()) {
            cb.onError(new IllegalArgumentException("uid is null/empty"));
            return;
        }
        ...
        String datePath;
        String dismissedPath;
        String formattedFieldTopLevel;

        switch (type) {
            case INSURANCE:
                datePath = "car.insuranceDateMillis";
                dismissedPath = "car.dismissedInsuranceStage";
                formattedFieldTopLevel = "formattedInsuranceDate";
                break;

            case TEST:
                datePath = "car.testDateMillis";
                dismissedPath = "car.dismissedTestStage";
                formattedFieldTopLevel = "formattedTestDate";
                break;

            case TREATMENT_10K:
                datePath = "car.treatmentDateMillis";
                dismissedPath = "car.dismissedTreatment10kStage";
                formattedFieldTopLevel = "formattedTreatDate";
                break;
        ...
        Map<String, Object> updates = new HashMap<>();
        updates.put(datePath, newDateMillis);
        updates.put(formattedFieldTopLevel, formattedDate);

        // reset dismissed stage so notifications can appear again for the new date
        updates.put(dismissedPath, null);
