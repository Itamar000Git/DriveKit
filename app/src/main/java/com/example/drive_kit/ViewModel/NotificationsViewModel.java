...
//            String dismissed = driver.getCar().getDismissedTestStage();
//
//            if (stage != NotificationItem.Stage.NONE &&
//                    (dismissed == null || !stage.name().equals(dismissed))) {
//
...
//            String dismissed = driver.getCar().getDismissedTreatment10kStage();
//
//            if (stage != NotificationItem.Stage.NONE &&
//                    (dismissed == null || !stage.name().equals(dismissed))) {
//
...
public class NotificationsViewModel extends ViewModel {
    ...
    public ArrayList<NotificationItem> buildNotifications(Driver driver, long now) {
        ...
        // ===== FIX: work with car once (and avoid null issues) =====
        Car car = driver.getCar(); // Driver.getCar() should ensure non-null (ensureCar)

        long oneYearMillis = TimeUnit.DAYS.toMillis(366);

        // --- INSURANCE ---
        long insuranceStart = car.getInsuranceDateMillis();
        if (insuranceStart > 0) {
            long insuranceEnd = insuranceStart + oneYearMillis;

            NotificationItem.Stage stage = calcStage(insuranceEnd, now);
            String dismissed = car.getDismissedInsuranceStage();
        ...
        // --- TEST ---
        long testStart = car.getTestDateMillis();
        if (testStart > 0) {
            long testEnd = testStart + oneYearMillis;

            NotificationItem.Stage stage = calcStage(testEnd, now);
            String dismissed = car.getDismissedTestStage();
        ...
        // --- TREATMENT 10K ---
        long treatStart = car.getTreatmentDateMillis();
        if (treatStart > 0) {
            NotificationItem.Stage stage = calcTreatStage(treatStart, now);
            String dismissed = car.getDismissedTreatment10kStage();
    ...
    private String messageFor(String label, NotificationItem.Stage stage) {
        switch (stage) {
            case D28:
                return "בעוד פחות מ 28 ימים יפוג תוקף " + label + " שלך";
            case D14:
                return "בעוד פחות מ 14 ימים יפוג תוקף " + label + " שלך";
            case D7:
                return "בעוד פחות מ 7 ימים יפוג תוקף " + label + " שלך";
            case D1:
                return "בעוד יום אחד יפוג תוקף " + label + " שלך";
            case EXPIRED:
                return "פג תוקף " + label + " שלך, נא לחדש בהקדם";

            case M6:
                return "עברו 6 חודשים מהטיפול האחרון. מומלץ לקבוע טיפול 10K";
            case M7:
                return "עברו 7 חודשים מהטיפול האחרון. מומלץ לקבוע טיפול 10K";
            case M8:
                return "עברו 8 חודשים מהטיפול האחרון. מומלץ לקבוע טיפול 10K";
            case EXPIRED_TREAT:
                return "עברו 9 חודשים מהטיפול האחרון. פג תוקף טיפול 10K, נא לטפל בהקדם";
    ...
    /**
     * calculates the stage based on end date and now
     */
    public static NotificationItem.Stage calcStage(long endMillis, long now) {
        long daysUntil = TimeUnit.MILLISECONDS.toDays(endMillis - now);
    ...
    public void deferNotification(String uid, NotificationItem item) {
        repo.deferNotification(uid, item, new NotificationsRepository.SimpleCallback() {
            ...
            @Override
            public void onError(Exception e) {
                // optional: toastMessage.postValue("שגיאה בהדחה");
            }
        });
    }
    ...
    /**
     * updates the service date for the given notification item for the given user
     * FIX: remove item immediately from UI (like defer), then reload from DB
     */
    public void doneNotification(String uid, NotificationItem item, long newDateMillis) {
        repo.doneButton(uid, item.getType(), newDateMillis, new NotificationsRepository.SimpleCallback() {
            @Override
            public void onSuccess() {
                ...
                // ===== FIX: remove immediately so it disappears right away =====
                ArrayList<NotificationItem> current = noty.getValue();
                if (current != null) {
                    ArrayList<NotificationItem> updated = new ArrayList<>(current);
                    updated.remove(item);
                ...
                // reload from db to sync (if logic says it should re-appear, it will)
                loadNoty(uid);
